...
for (const signal of ["Make a better ad.", "Know what it costs.", "Create an ad", "From brief to finished ad."]) {
...
if (ok) pass("homepage leads with a short UGC outcome");

const access = read("src/app/pricing/page.tsx");
for (const signal of ["Choose how you want to work", "Managed", "Private install", "Studio, REST, MCP, and CLI", "Approval before spend"]) {
...
for (const signal of ["Review scripts and cost", "Customer pain", "Product mechanism", "Estimated cost", "Credits required", "and render", "Receipt"]) if (!create.toLowerCase().includes(signal.toLowerCase())) fail(`create surface missing trust signal ${signal}`);
...
if (!analytics.includes("No results yet")) fail("analytics does not fail closed without real performance events");
