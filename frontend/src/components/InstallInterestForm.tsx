...
export function InstallInterestForm({ compact = false }: { compact?: boolean }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error("install_interest_failed");
      setState("success");
      setEmail("");
    } catch {
      setState("error");
  ...
  return (
    ...
    >
      ...
      <p className={`mt-2.5 text-[11px] ${state === "error" ? "text-red-700" : "text-black/45"}`}>
        {state === "error" ? "Couldn’t save that request. Try again." : "One-time private install. Provider, model, and hosting usage are separate."}
      </p>
    </form>
  );
}
