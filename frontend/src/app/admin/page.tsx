...
import { isDemoMode, verifyPassword } from "@/lib/api";
...
export default function AdminEntry() {
  ...
  useEffect(() => {
    if (isDemoMode()) router.replace("/admin/dashboard");
  }, [router]);
  ...
  return (
    <main className="flex flex-1 items-center justify-center px-6 py-24">
      <div className="w-full max-w-sm">
        ...
        <p className="mt-2 text-sm text-text-muted">Required only when the live backend is enabled.</p>
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          ...
          <button
            ...
            className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-accent px-4 py-3 text-sm font-medium text-white transition hover:bg-accent-dim disabled:opacity-50"
          >
