...
export default function CalendarPage() {
  ...
  return <StudioShell eyebrow="Publishing calendar">
    <PageHeader kicker="Calendar" title="Schedule a post." body="Choose an account, review the post and time, then approve." action={<StatusPill tone={receipt ? "green" : "neutral"}>{receipt ? "Receipt recorded" : "Nothing scheduled"}</StatusPill>} />
    <div className="grid gap-5 p-6 xl:grid-cols-[1fr_1fr] lg:p-10">
      <section className="rounded-[22px] border border-black/7 bg-white p-5 shadow-[0_16px_45px_rgba(0,0,0,.035)]">
        <div className="flex items-center justify-between"><div><p className="text-xs text-black/40">Schedule state</p><h3 className="mt-1 text-xl font-semibold tracking-tight">Scheduled posts</h3></div><CalendarDays className="h-5 w-5 text-black/30"/></div>
      ...
      <section className="rounded-[22px] border border-black/7 bg-[#f4f4f1] p-5">
        <div className="flex items-center justify-between"><div><p className="text-xs text-black/40">New post</p><h3 className="mt-1 text-xl font-semibold tracking-tight">Review and schedule</h3></div><ShieldCheck className="h-5 w-5 text-[#2357ff]"/></div>
