...
export default function LibraryPage() {
  ...
  return <StudioShell eyebrow="Creative library">
    <PageHeader kicker="My ads" title="Your ads." body="Finished renders and receipts appear here." action={<Link href="/studio/create" className="inline-flex items-center gap-2 rounded-xl bg-black px-4 py-2.5 text-sm font-medium text-white"><Plus className="h-4 w-4"/>Create</Link>} />
    <div className="p-6 lg:p-10">
      ...
      {loaded && jobs.length === 0 ? <div className="grid min-h-[360px] place-items-center rounded-[24px] border border-dashed border-black/10 bg-white p-8 text-center"><div><FileVideo2 className="mx-auto h-9 w-9 text-black/20"/><h2 className="mt-4 text-lg font-semibold">No ads yet</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-black/45">Create an ad. The finished file and receipt will appear here.</p><Link href="/studio/create" className="mt-5 inline-flex rounded-xl bg-black px-4 py-3 text-sm font-medium text-white">Create the first ad</Link></div></div> : null}
