  ...
  return <StudioShell eyebrow="Workspace settings">
    ...
    <div className="grid gap-5 p-6 xl:grid-cols-[1.1fr_.9fr] lg:p-10">
      ...
      <div className="space-y-5">
        ...
        <section className="rounded-[22px] border border-black/7 bg-white p-5"><div className="flex items-center gap-2"><KeyRound className="h-4 w-4 text-black/40"/><p className="text-xs font-medium">Ways to use Buffer Blaster</p></div><div className="mt-4 flex flex-wrap gap-2">{(status?.interfaces || ["ui","rest","mcp","cli","plugin","voice"]).map((item)=><span key={item} className="rounded-full border border-black/8 bg-[#fafaf8] px-3 py-2 text-xs font-medium uppercase">{item}</span>)}</div><p className="mt-4 text-xs leading-5 text-black/42">Verify providers in admin settings.</p><Link href="/admin/settings" className="mt-4 inline-flex rounded-xl bg-black px-4 py-2.5 text-xs font-medium text-white">Verify providers</Link></section>
      </div>
    </div>
  </StudioShell>;
}
