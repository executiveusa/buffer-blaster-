...
export default function CreateUGCPage() {
  ...
  return <StudioShell eyebrow="UGC ad factory">
    <PageHeader
      kicker="Create"
      title="Build your ad."
      body="Add the product details. Review the scripts and cost. Approve the render."
    ...
    <div className="grid gap-5 p-6 lg:grid-cols-[.82fr_1.18fr] lg:p-10">
      <section className="space-y-4 rounded-[22px] border border-black/7 bg-white p-5 shadow-[0_16px_45px_rgba(0,0,0,.035)]">
        ...
        <div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-[#2357ff]"/><h3 className="text-sm font-medium">Ad brief</h3></div>
        ...
        <button onClick={buildPlan} disabled={!canPlan || working !== null} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-black px-4 py-3 text-sm font-medium text-white disabled:opacity-45">{working === "plan" ? <Loader2 className="h-4 w-4 animate-spin"/> : <Sparkles className="h-4 w-4"/>}Review scripts and cost</button>
        <p className="text-[11px] leading-5 text-black/38">Planning is free.</p>
      ...
      <section className="space-y-4 rounded-[22px] border border-black/7 bg-[#f4f4f1] p-5">
        <div className="flex items-center justify-between"><div><p className="text-xs text-black/40">Your ad</p><h3 className="mt-1 text-xl font-semibold tracking-tight">Plan → approve → render</h3></div><StatusPill tone={finalState === "finished" ? "green" : plan?.gate.passed ? "blue" : "neutral"}>{finalState === "finished" ? "Finished" : plan?.gate.passed ? "Gate passed" : "Draft"}</StatusPill></div>
        ...
        {!plan ? <div className="grid min-h-[330px] place-items-center rounded-[20px] border border-dashed border-black/10 bg-white px-8 text-center"><div><ShieldCheck className="mx-auto h-8 w-8 text-black/25"/><h4 className="mt-4 text-base font-medium">Start with your brief</h4><p className="mx-auto mt-2 max-w-md text-xs leading-5 text-black/42">You will see the scripts and cost before rendering.</p></div></div> : <>
