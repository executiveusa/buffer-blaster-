...
export default function CampaignsPage() {
  ...
  return <StudioShell eyebrow="Campaign orchestration">
    <PageHeader kicker="Campaigns" title="Plan a campaign." body="Set the goal and channels. Review the plan before using it." action={<StatusPill tone={simulated ? "amber" : "green"}>{simulated ? "Simulation only" : "Human gate on"}</StatusPill>} />
    <div className="grid gap-5 p-6 xl:grid-cols-[.72fr_1.28fr] lg:p-10">
      <section className="rounded-[22px] border border-black/7 bg-white p-5 shadow-[0_16px_45px_rgba(0,0,0,.035)]">
        ...
        <button onClick={()=>void generate()} disabled={working || !brand.trim() || !objective.trim() || platforms.length === 0} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-black px-4 py-3 text-sm font-medium text-white disabled:opacity-40">{working?<Loader2 className="h-4 w-4 animate-spin"/>:<Sparkles className="h-4 w-4"/>}{working?"Planning…":"Build campaign plan"}</button>
