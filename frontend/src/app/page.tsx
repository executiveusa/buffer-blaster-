    ...
    <main className="flex-1">
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="relative mx-auto max-w-5xl px-6 py-28 sm:py-36">
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            <Link href="/create" className="inline-flex items-center gap-2 rounded-xl bg-accent px-6 py-3 text-sm font-medium text-white transition hover:bg-accent-dim">
              Open Creator Studio <ArrowRight className="h-4 w-4" aria-hidden />
            </Link>
            <a href="#how" className="inline-flex items-center gap-2 rounded-xl border border-border px-6 py-3 text-sm font-medium text-text transition hover:bg-bg-elevated">
              See how it works
            </a>
          </div>
      ...
      <section className="border-y border-border bg-bg-elevated">
        <div className="mx-auto grid max-w-5xl gap-8 px-6 py-20 sm:grid-cols-3">
          <div className="flex gap-3"><Search className="h-5 w-5 text-accent-soft"/><p className="text-sm text-text-muted"><span className="text-text">Find less, choose better.</span> Discovery returns the strongest few matches instead of thousands of raw prompts.</p></div>
          <div className="flex gap-3"><ShieldCheck className="h-5 w-5 text-success"/><p className="text-sm text-text-muted"><span className="text-text">Provenance stays attached.</span> Verified source and license metadata travel with exported recipes.</p></div>
          <div className="flex gap-3"><Download className="h-5 w-5 text-warning"/><p className="text-sm text-text-muted"><span className="text-text">Your work stays portable.</span> Export inspectable files and agent context instead of locking your process into one model.</p></div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-3xl px-6 py-24 text-center">
        ...
        <Link href="/create" className="mt-8 inline-flex items-center gap-2 rounded-xl bg-accent px-6 py-3 text-sm font-medium text-white transition hover:bg-accent-dim">
          Start creating <ArrowRight className="h-4 w-4" aria-hidden />
        </Link>
      </section>

      <footer className="border-t border-border">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-4 px-6 py-10 text-xs text-text-dim">
          <span>© {new Date().getFullYear()} Creator Studio</span>
          <Link href="/create" className="transition hover:text-text">Creator workspace</Link>
        </div>
      </footer>
    </main>
  );
