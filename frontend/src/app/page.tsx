...
export default function LandingPage() {
  return (
    <main className="min-h-screen bg-[#f4f3ef] text-[#151613]">
        ...
        <div className="grid gap-12 lg:grid-cols-[.86fr_1.14fr] lg:items-center lg:gap-16">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white px-3.5 py-2 text-[10px] font-semibold uppercase tracking-[0.15em] text-black/60">
              <Sparkles className="h-3.5 w-3.5 text-[#2357ff]" /> Private beta · coming soon
            </div>
            <h1 className="mt-6 max-w-3xl text-balance text-5xl font-semibold leading-[.92] tracking-[-0.075em] sm:text-7xl lg:text-[82px]">
              Find the angle.<br />Make the ad.<br /><span className="text-black/42">Learn what works.</span>
            </h1>
            <p className="mt-7 max-w-2xl text-balance text-base leading-7 text-black/62 sm:text-lg">
              Buffer Blaster is private creative infrastructure for teams and agents that need to turn product truth into testable UGC, keep paid actions governed, and bring real evidence back into the next creative decision.
            </p>
      ...
      <section className="border-y border-black/7 bg-[#e8e7e3] py-8">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-center gap-x-10 gap-y-4 px-5 text-[10px] font-semibold tracking-[0.14em] text-black/48">
          <span>ECOMMERCE</span><span>SHOPIFY</span><span>CLIENT TEAMS</span><span>AGENCIES</span><span>AI AGENTS</span>
        </div>
      </section>
      ...
      <section id="system" className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:py-28">
        <div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr] lg:items-start">
          <div className="lg:sticky lg:top-12">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-black/48">The creative loop</p>
            <h2 className="mt-4 max-w-xl text-4xl font-semibold leading-[1.01] tracking-[-0.06em] sm:text-5xl">One governed system from signal to evidence.</h2>
            <p className="mt-5 max-w-lg text-base leading-7 text-black/60">Research, reference analysis, creative planning, provider routing, repurposing, approvals, Shopify context, distribution handoff, and performance evidence can share one operating model instead of becoming eight disconnected tools.</p>
          </div>
      ...
      <section id="control" className="bg-[#10110f] text-white">
        <div className="mx-auto grid max-w-7xl gap-10 px-5 py-20 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:py-28">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/48">Control stays with the operator</p>
            <h2 className="mt-4 max-w-2xl text-4xl font-semibold leading-[1.01] tracking-[-0.06em] sm:text-5xl">Automation should remove busywork, not remove judgment.</h2>
            <p className="mt-5 max-w-xl text-base leading-7 text-white/58">The system can research, draft, route, organize, and prepare the next move. Paid generation, publishing, and ad activation remain explicit decisions, with limits enforced on the server.</p>
            <div className="mt-7 flex flex-wrap gap-2 text-[10px]"><span className="rounded-full bg-white/8 px-3 py-2">PLAN</span><span className="rounded-full bg-white/8 px-3 py-2">REVIEW</span><span className="rounded-full bg-white/8 px-3 py-2">APPROVE</span><span className="rounded-full bg-[#b9ff66] px-3 py-2 text-black">LEARN</span></div>
          </div>
          <div className="rounded-[22px] border border-white/10 bg-white/[.055] p-6">
            <div className="flex items-center gap-2"><ReceiptText className="h-4 w-4 text-[#b9ff66]" /><p className="text-sm font-medium">Every consequential job keeps its evidence</p></div>
            <div className="mt-6 space-y-4 text-xs"><ReceiptRow label="Plan" value="what we are testing" /><ReceiptRow label="Rights" value="what can be used commercially" /><ReceiptRow label="Approval" value="who cleared the action" /><ReceiptRow label="Cost" value="what the action may spend" /><ReceiptRow label="Result" value="what actually happened" /></div>
          </div>
        </div>
      </section>
      ...
      <section id="agents" className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:py-28">
        <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-black/48">Agent-native</p>
            <h2 className="mt-4 max-w-xl text-4xl font-semibold leading-[1.01] tracking-[-0.06em] sm:text-5xl">Use the Studio. Or let your agent call the same system.</h2>
            <p className="mt-5 max-w-xl text-base leading-7 text-black/60">The browser is one doorway. REST, MCP, and CLI let approved agents use the same governed workflow without creating a second process or a second source of truth.</p>
          ...
          <div className="rounded-[22px] border border-black/8 bg-white p-6 shadow-[0_22px_70px_rgba(0,0,0,.06)]">
            <div className="flex items-center gap-2 text-xs text-black/50"><Bot className="h-4 w-4 text-[#2357ff]" />Agent request</div>
            <p className="mt-5 text-xl leading-7 tracking-[-0.03em]">Analyze this reference. Build three original angles. Show me the route, rights, estimated cost, and plan before anything paid happens.</p>
            <div className="mt-6 grid gap-2 sm:grid-cols-2"><TrustCard title="Prepare" body="No-spend research and planning can run automatically." /><TrustCard title="Commit" body="Consequential actions remain governed and explicit." /></div>
          </div>
        </div>
      </section>

      <section id="beta" className="mx-auto max-w-6xl px-5 pb-24 sm:px-8">
        <div className="overflow-hidden rounded-[30px] bg-[#dfff67] px-6 py-12 sm:px-10 lg:px-12 lg:py-14">
          ...
          <div className="mt-4 grid gap-8 lg:grid-cols-[1fr_.9fr] lg:items-end">
            <div>
              <h2 className="max-w-3xl text-4xl font-semibold tracking-[-0.06em] sm:text-5xl">We’re opening Buffer Blaster to a small group of teams first.</h2>
              <p className="mt-4 max-w-2xl text-sm leading-6 text-black/62">Join the list for beta access, build notes, and the first public demos. The production system stays private while we finish the final release gauntlet.</p>
            </div>
      ...
      <footer className="border-t border-black/7 px-5 py-8 text-xs text-black/50 sm:px-8">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><span>Buffer Blaster · Private creative infrastructure</span><div className="flex gap-5"><Link href="#beta">Beta</Link><Link href="/studio">Studio</Link><Link href="/blog">Notes</Link></div></div>
      </footer>
    </main>
  );
}
...
function DeviceStage() {
  return (
    <div className="relative mx-auto min-h-[500px] w-full max-w-[760px] sm:min-h-[600px] lg:min-h-[650px]" aria-label="Creative outputs inside Buffer Blaster">
      ...
      <div className="absolute left-[3%] right-[3%] top-[5%] aspect-[16/10] rounded-[24px] border-[8px] border-[#1b1c19] bg-[#11120f] shadow-[0_38px_90px_rgba(0,0,0,.18)] sm:border-[10px]">
        <div className="relative h-full w-full overflow-hidden rounded-[14px] bg-[#151713] sm:rounded-[16px]">
          <video className="h-full w-full object-cover" autoPlay muted loop playsInline preload="metadata" poster="/media/ugc-hero-poster.svg"><source src="/media/ugc-skincare.mp4" type="video/mp4" /></video>
          <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/45 via-transparent to-black/5" />
          <div className="pointer-events-none absolute bottom-5 left-5 right-5 flex items-end justify-between text-white sm:bottom-7 sm:left-7 sm:right-7"><div><p className="text-[9px] font-semibold uppercase tracking-[0.2em] text-white/65">Creative 01</p><p className="mt-1 text-base font-medium tracking-[-0.02em] sm:text-xl">Skincare · problem-led UGC</p></div><span className="hidden rounded-full border border-white/20 bg-black/25 px-3 py-1.5 text-[9px] font-medium backdrop-blur sm:block">GOVERNED OUTPUT</span></div>
        </div>
      ...
      <div className="absolute bottom-[1%] right-[4%] w-[30%] min-w-[132px] max-w-[190px] rounded-[30px] border-[7px] border-[#151613] bg-[#151613] p-[3px] shadow-[0_32px_70px_rgba(0,0,0,.28)] sm:bottom-[2%] sm:right-[7%] sm:rounded-[38px] sm:border-[8px]">
        <div className="relative aspect-[9/18.8] overflow-hidden rounded-[20px] bg-[#11120f] sm:rounded-[27px]">
          <video className="h-full w-full object-cover" autoPlay muted loop playsInline preload="metadata" poster="/media/ugc-hero-poster.svg"><source src="/media/ugc-streetwear.mp4" type="video/mp4" /></video>
