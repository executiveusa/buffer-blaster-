...
This is the missing bridge between the deterministic factory plan and a final
asset. It preserves provider neutrality: model IDs remain in environment-backed
media providers, while this service owns approvals, spend authorization,
continuity, local QA, durable receipts, and final asset delivery.
"""
...
def extract_video_url(payload: Any) -> str | None:
    """Find a likely generated video URL in common provider response shapes."""
...
def _provider_state(payload: Any) -> str:
    if not isinstance(payload, dict):
        return "unknown"
    raw = payload.get("status") or payload.get("state") or payload.get("queue_status") or ""
...
async def _submit_and_download(
    *,
    ...
    receipt = await provider.submit_video(
        prompt=prompt,
        image_url=image_url,
        duration="10",
        aspect_ratio="9:16",
    ...
    completed = await _wait_for_video(
        provider,
        receipt,
    ...
    if not downloaded.get("ok"):
        return {
            "ok": False,
            "error": "generated_video_download_failed",
            "clip": clip_number,
            "receipt": receipt,
            "download": downloaded,
        }
    return {
        ...
        "clip": clip_number,
        "receipt": receipt,
        "provider_response": completed.get("provider_response"),
...
async def execute_ugc_factory_ad(
    ...
    *,
    ...
    try:
        ...
        clip1 = await _submit_and_download(
            provider=provider,
            storage=storage,
            prompt=plan["clips"][0]["prompt"],
            image_url=None,
            clip_number=1,
            workdir=workdir,
        ...
        async def render_second(attempt: int) -> dict[str, Any]:
            ...
            return await _submit_and_download(
                provider=provider,
                storage=storage,
                prompt=plan["clips"][1]["prompt"],
                image_url=seed_asset["signed_url"],
                clip_number=2,
                workdir=workdir,
                poll_interval_seconds=poll_interval,
                timeout_seconds=timeout,
        ...
        if seam >= seam_threshold:
            # The plan budgets three paid generation calls by default, allowing
            # one bounded clip-2 retry without an unbounded repair loop.
            ...
            if seam >= seam_threshold:
                ...
                return {
                    "ok": False,
                    "error": "seam_qa_failed",
                    "state": "seam_qa_failed",
                    "job_id": job_id,
                    "allowance": allowance,
                    "seam_diff": seam,
        ...
        qa = {
            "seam_diff": seam,
            "seam_threshold": seam_threshold,
            "seam_passed": seam < seam_threshold,
            "audio_present_in_both_clips": bool(stitched.get("audio")),
            "paid_generation_calls": len(clip_receipts),
        }
        final_result = {
            "ok": state == "finished",
            "state": state,
            "job_id": job_id,
            "factory_version": plan.get("factory_version"),
            "allowance": allowance,
            "qa": qa,
            "final_asset": final_asset,
            "provider_receipts": clip_receipts,
            "approval_required_before_publish": True,
        }
        await update_job(
            job_id,
            state=state,
            provider_receipt={"clips": clip_receipts},
            output={"final_asset": final_asset, "qa": qa, "allowance": allowance},
        )
        return final_result
    finally:
