...
Agents use this server for the campaign/UGC layer. Public publishing remains an
optional downstream integration behind the same explicit approval contracts.
"""
...
MCP_TOOLS: list[dict[str, Any]] = [
    ...
    {"name": "render_ugc_ad_factory_clip", "description": "Submit one gated UGC factory clip to the configured media provider. `approved: true` is required because this can create a paid render.", "inputSchema": {"type": "object", "required": ["product", "audience", "pain", "mechanism", "approved"], "properties": {"product": {"type": "string"}, "audience": {"type": "string"}, "pain": {"type": "string"}, "mechanism": {"type": "string"}, "offer": {"type": "string"}, "platform": {"type": "string"}, "actor_description": {"type": "string"}, "delivery_tone": {"type": "string"}, "visual_lane": {"type": "string"}, "clip_number": {"type": "integer", "minimum": 1, "maximum": 2}, "image_url": {"type": ["string", "null"]}, "approved": {"type": "boolean"}}}},
...
async def mcp(request: Request) -> JSONResponse:
    ...
    try:
        ...
        elif name == "render_ugc_ad_factory_clip":
            render_args = dict(args)
            clip_number = int(render_args.pop("clip_number", 1))
            approved = bool(render_args.pop("approved", False))
            image_url = render_args.pop("image_url", None)
            value = await render_ugc_factory_clip(
                UGCFactoryBrief(**render_args),
