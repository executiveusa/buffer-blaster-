...
Examples:
  ...
  python -m cli.blaster shopify-context product.json
  python -m cli.blaster shopify-context-get <receipt-id>
...
def _help() -> None:
    print(
        "blaster <status|pricing|campaign|ugc-prompt|ugc-plan|ugc-plan-create|ugc-plan-get|reference-analyze|reference-strategy|provider-capabilities|provider-route|repurpose-plan|repurpose-get|shopify-context|shopify-context-get|ugc-execute|wallet|jobs|job|accounts|schedule|mcp-info> [json-file-or-id]"
    )
...
def main() -> int:
    ...
    elif command == "provider-capabilities":
